...
uint8_t send_data_with_get( DL_SEND_PARAMS * const pPara )
{
	...
	snprintf_P(szBuf+l1,BUFFER_SIZE-l1,PSTR("#TIME=%lu&B=%u&Tmcu=%u&CO=%ld&COe=%d&COn=%u&NO2=%ld&NO2e=%d&NO2n=%u&O3=%ld&O3e=%d&O3n=%u") ,
	ds.data_timestamp,
	ds.vBat,
	ds.tempMCU,
	ds.gases.gas[0],
	ds.gases.error_check[0],
	ds.gases.readingNumber[0],
	ds.gases.gas[1],
	ds.gases.error_check[1],
	ds.gases.readingNumber[1],
	ds.gases.gas[2],
	ds.gases.error_check[2],
	ds.gases.readingNumber[2]
	);
...
uint16_t send_data_with_post( DL_SEND_PARAMS * const pPara )
{
	...
	while(iter<QUEUE_LEN) {
		...
		PSTR("#TIME=%lu&B=%u&Tmcu=%u&CO=%ld&COe=%d&COn=%u&NO2=%ld&NO2e=%d&NO2n=%u&O3=%ld&O3e=%d&O3n=%u") ,
		ds.data_timestamp,
		ds.vBat,
		ds.tempMCU,
		ds.gases.gas[0],
...
static uint8_t  dl_snapshot_make(const uint32_t ts)
{
	...
	volatile SENSIT_STATS * const ls = &p_item->gases;
	sens_it_getStats(ls);
	/////////////////////////////////////////////////////////////////////////////////////////////////////
	p_item->data_timestamp = (ts==-1)?rtc_get_time():ts;
	p_item->flags = 0;
...
void datalogger_run(void)
{
	while(1) {
		...
		sens_it_triggerReading('C');
		sens_it_triggerReading('D');
		sens_it_triggerReading('B');
		//////////////////////////////////////////////////////////
